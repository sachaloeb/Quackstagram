...
public class NotificationsUI{
    ...
    private void initializeUI() {
        ...
        String query = "SELECT * FROM Notifications WHERE notif_receiver = ?";
        try(Connection connection = SQLDBConnection.getConnection();
            PreparedStatement statement = connection.prepareStatement(query)){


            statement.setString(1, User.currentUser);
            ResultSet resultSet = statement.executeQuery();

            while (resultSet.next()) {
                String notificationMessage;
                String timestamp = resultSet.getString("notif_timestamp").trim();

                switch (resultSet.getString("notif_type").trim()) {
                    case "like":
                        String userWhoLiked = resultSet.getString("concerned_user").trim();
                        String imageId = resultSet.getString("liked_picture").trim();

                        notificationMessage = userWhoLiked + " liked your picture " + imageId + " " + getElapsedTime(timestamp) + " ago";
                        break;
                    case "follow":
                        String userWhoFollowed = resultSet.getString("concerned_user").trim();
